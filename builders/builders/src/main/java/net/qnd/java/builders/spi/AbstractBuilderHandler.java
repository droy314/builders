...
public abstract class AbstractBuilderHandler {
	...
	protected AbstractBuilderHandler(BuilderFactory factory,Class<?> builderType, Object parentBuilder) {
	...
	protected Object stubCall(Object proxy, Method method, Object[] args)
			throws Throwable, IllegalAccessException, InvocationTargetException {
				...
				if ("end".equals(methodName)) {
					...
					Command cmd = Commands.setPropertyValue(methodName, propertyType, value);
					addCommand(cmd);
					return proxy;
					...
					Command cmd = Commands.setPropertyWithBuilderOutput(methodName, nestedBuilder);
					addCommand(cmd);
